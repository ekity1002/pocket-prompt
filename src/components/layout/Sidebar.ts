...
export function createSidebar(props: SidebarProps): HTMLElement {
  ...
  if (props.onToggle) {
    ...
    toggleButton.className = 'p-1 text-gray-400 hover:text-gray-600 focus:outline-none focus:ring-2 focus:ring-primary-500 rounded';
...
function createSidebarItem(item: SidebarItem, isCollapsed?: boolean): HTMLElement {
  ...
  if (!isCollapsed) {
    ...
    if (item.badge) {
      ...
      badge.className = 'ml-2 px-2 py-1 text-xs font-medium bg-primary-100 text-primary-800 rounded-full';
...
function getSidebarItemClasses(item: SidebarItem): string {
  const baseClasses = 'w-full px-3 py-2 text-sm font-medium rounded-lg transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-primary-500';
