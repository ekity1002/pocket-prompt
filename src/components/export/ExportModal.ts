...
function createFormatSection(
  ...
  formatOptions.forEach((item) => {
    ...
    icon.className =
      'flex-shrink-0 w-8 h-8 bg-gray-100 rounded flex items-center justify-center text-sm font-bold mr-3';
...
function createOptionsSection(
  ...
  filenameInput.className =
    'w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 text-sm';
...
function createCheckboxOption(props: CheckboxOptionProps): HTMLElement {
  ...
  checkbox.className =
    'mt-1 mr-3 h-4 w-4 text-primary-600 border-gray-300 rounded focus:ring-primary-500';
...
function getFormatOptionClasses(selected: boolean): string {
  const baseClasses =
    'w-full border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 transition-colors duration-200';
