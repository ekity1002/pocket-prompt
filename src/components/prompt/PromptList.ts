...
function createLoadingState(): HTMLElement {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    for (let j = 0; j < 3; j++) {
      ...
      line.className = `h-3 bg-gray-200 rounded ${
        j === 2 ? 'w-1/2' : 'w-full'
      }`;
...
function createEmptyState(empty?: PromptListProps['empty']): HTMLElement {
  ...
  if (empty?.action) {
    ...
    actionButton.className = 'px-4 py-2 text-sm font-medium text-white bg-primary-600 hover:bg-primary-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500';
