...
export function createPromptSearch(props: PromptSearchProps): HTMLElement {
  ...
  searchInput.className =
    'w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500';
  ...
  if (props.onClear) {
    ...
    clearButton.className =
      'px-4 py-2 text-sm font-medium text-gray-600 hover:text-gray-800 focus:outline-none focus:ring-2 focus:ring-primary-500 rounded-lg';
  ...
  if (props.availableTags && props.availableTags.length > 0) {
    ...
    function updateTagFilters() {
      ...
      props.availableTags?.forEach((tag) => {
        ...
        tagButton.className = getTagButtonClasses(searchOptions.tags?.includes(tag) || false);
...
function getTagButtonClasses(active: boolean): string {
  const baseClasses =
    'px-2 py-1 text-xs font-medium rounded-full transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-primary-500';
